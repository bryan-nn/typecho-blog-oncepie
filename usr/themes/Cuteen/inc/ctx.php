...
{
    //XSS过滤
    public static function XssDel($data)
    {
        // Fix &entity\n;
    ...
    private static function MobileArchiverSort($var)
    {
        ...
        while ($var->next()) {
            if ($var->levels === 0) {
                $children = $var->getAllChildren($var->mid);
                if (empty($children)) {
                    $sort .= '<li><a onclick="Cuteen.bodyClose();" href="' . $var->permalink . '">' . self::IconArray()[0][$i] . $var->name . '</a></li>';
                } else {
                    ...
                    foreach ($children as $mid) {
                        $child = $var->getCategory($mid);
                        $sort .= '<div><a onclick="Cuteen.bodyClose();" href="' . $child['permalink'] . '">' . $child['name'] . '</a></div>';
                        $i--;
                    }
                    $sort .= '</div>';
                }
    ...
    private static function CardStyleArchives($ctx)
    {
        ...
        while ($ctx->next()) {
            ...
            $section .= '<a href="' . $ctx->permalink . '"><img data-src="' . self::ImageEcho($ctx) . '" class="card-style-img lazy"></a>';
