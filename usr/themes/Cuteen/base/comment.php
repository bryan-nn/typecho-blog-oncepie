    ...
    <div id="<?php $comments->theId(); ?>" class="comment-card <?= $commentLevelClass ?>">
        ...
        <div class="comment-info">
            ...
            <div class="comment-meta">
                <span class="comment-time"><?= App::timeSince($comments->created); ?>&nbsp;&nbsp;<?= Ctx::CommentStatus($comments) ?></span>
                ...
                <span id="cancel-comment-reply" class="cancel-comment-reply cl-<?php $comments->theId(); ?> comment-reply-link" style="display:none"><?php $comments->cancelReply('取消<svg class="icon ml-1" aria-hidden="true" id="darkMode"><use xlink:href="#x-circle"></use></svg>'); ?></span>
            </div>
            <div class="comment-content" data-expand="查看更多...">
                <p><?= Ctx::XssDel(Ctx::Comment($comments->content)) ?></p>
            </div>
        </div>
...
<div class="comment-input-warp" id="<?php $this->respondId(); ?>">
    <form id="comment-form" method="post" role="form" data-action="<?php $this->commentUrl() ?>">
        <div class="row">
            ...
            <?php else : ?>
                <input id="author" name="author" placeholder="昵称" class="comment-input col-sm-4" type="text" value="<?php $this->remember('author'); ?>" required>
                <input id="mail" name="mail" placeholder="邮箱" class="comment-input col-sm-4" type="email" value="<?php $this->remember('mail'); ?>" <?php if ($this->options->commentsRequireMail) : ?> required<?php endif; ?>>
                <input id="url" name="url" placeholder="网址(https://)" class="comment-input col-sm-4" type="text" value="<?php $this->remember('url'); ?>" <?php if ($this->options->commentsRequireURL) : ?> required<?php endif; ?>>
            <?php endif; ?>
        </div>
        <div class="comment-edit">
            <textarea id="comment-textarea" class="comment-textarea comment-input" name="text" placeholder="来都来了，说一句吧~" required></textarea>
        </div>
