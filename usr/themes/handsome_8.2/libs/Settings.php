...
//        $version = (string)self::version;
        ...
        if (Handsome::getPluginVersion("Handsome_Plugin", "Handsome") !== Handsome::version){
            echo '<script>alert("Handsome插件 最新版本' .Handsome::version .'没有上传到typecho的 /usr/plugins目录，请上传或者更新插件文件再重新禁用启用Handsome插件一次")</script>';
        }else{
            if (!Handsome::getPluginCorrect()){
                echo '<script>alert("Handsome 插件没有启用成功，确保Handsome插件文件夹名称首字母大写，修改后再次重新启用插件")</script>';
            }
        }
