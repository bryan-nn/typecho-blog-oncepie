...
//        $color = self::getThemeColor()[3];
        ...
        if ($options->WechatPic != null) {
            ...
            $returnHtml .= '<li class="nav-item '.$active.'" role="presentation"><a class="nav-link " style="" data-toggle="tab"  role="tab" data-target="#wechatpay_author"><i class="iconfont icon-wechatpay" aria-hidden="true"></i>微信</a></li>';
        }
        $returnHtml .='        </ul>
...
EOF;
    ...
    public static function parseGlobalPlayer()
    {
        ...
        $setting = [
            ...
            "fix_position"=>'true',
            "preload" => 'false',
            "autoplay" => in_array('autoPlay', Utils::checkArray($options->playerSetting)),
...
'."\n" . $content;
        ...
        //延迟加载
        if (in_array('lazyload', Utils::checkArray( $options->featuresetup)) && Content::getPostParseWay() != "vditor") {//对图片进行处理
            $placeholder = Utils::choosePlaceholder($options);//图片占位符
            $style = "";
            if ($isImagePost) {
                $style = ' style="width:100%;height=100%"';
            }
            $content = Utils::handle_preg_replace_callback('/<img (.*?)src(.*?)(\/)?>/',function ($matches)use ($placeholder,$style){
                return '<img '.$matches[1].'src="' . $placeholder . '"' . $style . ' data-original'.$matches[2].'/>';
                },$content);
...
EOF;
	...
	/*prevent music api abuse:$music_api_verification=gAAAAABh4tagBme1VLh3slLPi7778ZsIKOWnfI4IV7bEVjNsVkbjWG_qKGbJ6G665mZ899qsTN_euXr4RhAmSPnLCgsai8oLPQ==*/
            }
        } else {
            $atts = ltrim($text);
        }
