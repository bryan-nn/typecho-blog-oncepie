...
<code>rgba(0,0,0,0.3)</code>，填写后你就会发现效果的，就是让透明色不太过透明，导致字都看不清了。</br></br> 一般来说，".redText('根据背景图片的主色调来填比较好看。')." </br>举例：背景图片主色调是蓝色，那就填 <code>rgba(56, 136, 255, 0.3)</code> (56,136,255) 就是蓝色，0.3 代表背景的透明度。"));
    ...
    $dark_codeStyle = new Select('dark_codeStyle',
        array(
            ...
            'vs' => 'vs',
            'xcode' => 'xcode',
        ),'mac_dark','深色模型下的代码高亮风格选择','使用的代码高亮风格见 <a href="https://highlightjs.org/">风格列表</a>');
    ...
    $codeStyle = new Select('codeStyle',
        array(
            ...
            'vs' => 'vs',
            'xcode' => 'xcode',
        ),'mac_light','浅色模型下代码高亮风格选择','使用的代码高亮风格见 <a href="https://highlightjs.org/">风格列表</a>');
...
QQ音乐：如歌单地址： <code>https://y.qq.com/n/yqq/playlist/1144188779.html</code></br>3. 虾米音乐：如歌单地址： <code>http://www.xiami.com/collect/254478782</code></br>4. 百度音乐：如歌单地址： <code>http://music.baidu.com/songlist/364201689</code></br>');
    ...
    //七牛云镜像存储
    $cdnAddress = new Textarea('cdn_add', NULL, NULL, _t('本地资源云存储(镜像)加速'), _t('通过各大服务商的镜像存储功能，将博客的本地图片（即在本服务器上的图片）自动替换为对应镜像空间的图片地址，而在第一次访问的时候，会自动将本地服务器图片上传到镜像空间（需要在服务商进行配置 <code>加速域名</code>）。</br>而通常访问云服务器的速度要比本服务器速度要快（因为云服务器具有cdn特性）。</br> 填写内容格式为 <code>自定义域名 | CDN
类型</code> </br> 举例： <code>https://assets.ihewro.com | UPYUN</code> 在服务商的镜像空间配置加速域名为 <code>https://www.ihewro.com</code> </br>   支持以下服务商：</br>七牛云「QINIU」</br>又拍云「UPYUN」</br>阿里云「ALIOSS」</br> 腾讯云「QCLOUD」具体细节请看 <a href="https://auth.ihewro.com/user/docs/#/preference/speed">详细使用文档</a></br><b style="color:red;">如果在Handsome插件中开启了前端使用Vditor.js 解析，则文章的图片暂时无法替换</b>'));
    $form->addInput($cdnAddress);

    $LocalResourceSrc = new Text('LocalResourceSrc', NULL, NULL, _t('将主题静态资源上传到你的cdn上'),_t('使用该项设置前，你必须有自己搭建的cdn服务器（不是指当前服务器）<a href="https://auth.ihewro.com/user/docs/#/preference/speed">详细使用文档</a></br> 主题目录下的<code>/assets/</code>目录下有 
